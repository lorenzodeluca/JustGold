        ...
        return (
            <div className="card" style={{ width: '18rem', backgroundColor: '#252b48', borderRadius: '10px' }}>
                <div className="card-body">
                    ...
                    <h6 className="card-subtitle mt-2 text-muted" style={{ marginBottom: '-5px', padding: '0' }}><small>{(this.props.info=="")?'\u00A0':this.props.info}</small></h6>
                    <p className="card-text"><h4 style={{ margin: '0', padding: '0' }}>{this.props.value}</h4>
                        <h4 style={{ margin: '0', padding: '0' }}>{(this.props.unit=="")?'\u00A0':this.props.unit}</h4>
                        {this.props.percentage > 0 ?
                            <font color="#00ff15">↑{this.props.percentage}%</font>
                            : <font color="red">↓{this.props.percentage.replace("-","")}%</font>}<p></p>
                        <h6>{this.props.extra}</h6>
                    </p>

                    { this.props.hidebutton!=="true"?<div className="row justify-content-between">
